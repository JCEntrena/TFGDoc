...
\begin{proof}
  ...
  respectivamente. En $S^n - (A^+ \cap A^-)$ tomamos la descomposición $U = S^n - A^+, U = S^n - A^-$. Así,
  ...
  Como $A^+ \cap A^- \cong I^{k-1}$, por hipótesis de inducción $H_p(S^n - (A^+ \cap A^-)) \cong \begin{cases} 0 &\text{ si } p > 0 \\
  ...
  \end{tikzcd} \]
  Como $H_p(S^n - \bigcap\limits_{i = 1}^\infty A_i) = \begin{cases} 0 &\text{ si } p > 0 \\ \Z &\text{ si } p = 0 \end{cases}$ por inducción,
  entonces $0 = i_*(\beta) = 0 = [i_\#(\beta)]$, luego existe $d \in S_{p+1}(\bigcup\limits_{i = 1}^\infty A_i)$ con $\partial d = i_\#(\beta)$.
