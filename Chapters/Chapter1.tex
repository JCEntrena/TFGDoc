...
$\partial d = \partial c + \partial a \implies \overline{\partial d} = \overline{\partial c} + \overline{\partial a} =  $ % TODO
...
$i_*$ no tiene por qué ser un monomorfismo, y $\pi_*$ no tiene por qué ser un epimorfismo, aunque se verifica:
...
\begin{proposition}
  Para todo $p \geq 1$ existe un homomorfismo $\Delta \colon H_p(\X, A) \to H_{p-1}(\X, A)$ tal que $\Ker \Delta = \Img \pi_*$ y $\Img \Delta = \Ker i_*$
  ...
  se le llama la \underline{\smash{sucesión exacta del par $(\X, A)$.}}
\end{proposition}
...
\begin{proposition}[Naturalidad de la sucesión exacta]
  Sea $f \colon (\X, A) \to (\Y, B)$ una aplicación de pares. Entonces el diagrama siguiente es conmutativo:
