...
\begin{proof}
  ...
  Vamos a construir un homomorfismo $\mathcal T: \SX{p} \to \SX{p+1}$ tal que $\mathcal T \partial + \partial \mathcal T = Id_{\SX{p}}$ para $p > 0$.

  Si $[c] \in H_p(\X), c \in Z_p(\X)$, luego $\partial(c) = 0$ y $c \in S_p(\X)$. En consecuencia, $c = (\partial \mathcal T + \mathcal T \partial)(c)
  = \partial(\mathcal T(c))$ con $\mathcal T(c) \in \SX{p+1}$. Por tanto, $[c] = 0$, y eso implica que $H_p(\X) = 0 \quad \forall p > 0$.
  ...
  Queda claro que $\partial_0(T(\phi)) = \phi$. Si representamos por $\mathcal T$ el homomorfismo extensión de $T$ a $\SX{p}$, tenemos construído
  \[ \mathcal T \colon \SX{p} \to \SX{p+1} \hspace{0.5em} \text{tal que } \partial_0 \circ \mathcal T = I \]
  Si $p = 0 \implies i = 1$ y $\partial_1(\mathcal T(\phi)) = x_0$

  Si $1 \leq i \leq p+1$, entonces:
  \begin{align*}
    &\partial_i(\mathcal T(\phi)=(t_0,\dots,t_p)) = \mathcal T(\phi)(t_0, \dots, t_{i-1} \circ t_i, \dots, t_p) \\
    ...
    &= \mathcal T(\partial_{i-1} \phi)(t_0, \dots, t_p)
  \end{align*}
  Como consecuencia:
  \begin{align*}
    \partial(\mathcal T(\phi)) &= \sum\limits_{i = 1}^{p+1} (-1)^i \partial_i \mathcal T(\phi) =
    \phi + \sum\limits_{i = 1}^{p+1} (-1)^i \mathcal T \partial_{i-1}(\phi) = \\
    &= \phi - \sum\limits_{j = 0}^p (-1)^j \mathcal T \partial_j(\phi) = \phi - \mathcal T \partial(\phi)
  \end{align*}

  De donde se obtiene directamente que $\partial \mathcal T + \mathcal T \partial = Id$
